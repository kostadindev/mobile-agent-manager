...
\title{AgentFlow: Designing and Evaluating a Mobile Multimodal Interface for Controlling Personal AI Agents On the Go}
...
AgentFlow is a fully functional prototype (React client, FastAPI server; full architecture in Appendix~\ref{app:architecture}--\ref{app:structure}). Because AI behaviour is emergent and opaque---unlike traditional deterministic software where outputs are fully specified---we focused prototyping not on pixel-perfect UI but on making \emph{invisible AI processes legible}: system intent (what the orchestrator plans to do), system limitations (uncertainty cues), user feedback loops (approval gates), and user mental models (the DAG visualisation)~\cite{Yang2019}. The core idea is \textbf{bring-your-own-agents}: users register any agent by defining its role, tools, and metadata via the Agents tab or the REST API. The orchestrator dynamically reads the current agent pool for every request, so adding, removing, or reconfiguring agents requires no code changes. Users interact via three modalities suited to mobile, on-the-go use: \textbf{text} chat, \textbf{voice} recording (transcribed by Whisper---ideal for hands-free queries while walking), and \textbf{image} capture (analysed by GPT-4.1 vision---e.g., photographing a conference poster to trigger a literature search). All modalities are unified into a single planning prompt. The orchestrator decomposes the request into a dependency graph of tasks across the user's personal agent pool; execution streams real-time updates via Server-Sent Events.
...
HCAI demands \emph{inclusivity}---designing for ``us'' rather than ``them,'' embracing diversity in age, language, culture, and ability level~\cite{Capel2023}. Costanza-Chock's~\cite{CostanzaChock2020} design justice framework goes further, arguing that design processes must centre the voices of those most affected by a system's failures---in our case, users with limited English proficiency, motor impairments, or constrained mobile data. The UI supports four languages (English, Chinese, Arabic with full RTL layout, Bulgarian), three themes (dark, light, auto), ARIA labels on all icon buttons, \texttt{aria-live} regions for screen readers, and \texttt{focus-visible} keyboard styles. Voice and image are optional parallel pathways, not replacements for text, ensuring users with different abilities or devices can participate fully. This addresses HCAI's contextual awareness principle: the system adapts to the user's environment (modality choice) and respects boundaries (consent dialogs) rather than imposing a single interaction pattern.

\subsection{Three Design Configurations}
\label{sec:configs}

Our independent variable is \textbf{transparency level}, operationalised as three configurations (Table~\ref{tab:transparency}):
...
\begin{table}[h]
  ...
  \begin{tabular}{p{0.18\columnwidth} p{0.72\columnwidth}}
    ...
    \midrule
    C1: Black Box & Query in, final answer out. No plan card, no graph, no approval step. Agents auto-execute. \\
    C2: Plan Preview & Orchestrator's task plan shown as a card. User approves with one tap. A spinner indicates execution (no live graph). \\
    C3: Full Transparency & Plan card \emph{plus} a live DAG graph with animated node/edge status, step-by-step results streaming in real time, and checkpoint approval gates for sensitive actions. \\
    \bottomrule
  \end{tabular}
\end{table}

These configurations operationalise Shneiderman's~\cite{Shneiderman2020} two-dimensional framework: C1 sits at high automation / low human control, C3 at high automation / high human control, and C2 in between. This lets us test the trade-off between V1~(efficiency) and V2~(control) while keeping modality support constant across conditions.
...
\begin{figure}[h]
  ...
  \caption{AgentFlow system architecture. The mobile React client communicates with the FastAPI server over REST and SSE. The server delegates planning to a CrewAI orchestrator and streams execution updates back to the client in real time.}
...
\begin{table*}[t]
  ...
  \begin{tabular}{p{0.14\textwidth} p{0.08\textwidth} p{0.2\textwidth} p{0.5\textwidth}}
    ...
    \texttt{wiki\_search} & Wikipedia & \texttt{query}: str & Searches the Wikipedia API; returns up to 5 results with snippets. User-Agent: \texttt{AgentFlow/1.0}. Timeout: 10\,s. \\
